...
module.exports = function (robot) {
  ...
  const bloodSet = createDocumentSet(robot, 'blood', {
    ...
    nullBody: 'Blood Down the Gullet have not sung about anything that matches that query. Yet.'
  })
  ...
  robot.respond(/pour blood down our gullets/i, async msg => {
    ...
    try {
      ...
      msg.reply(`:metal: Our gullets have accepted ${tracks.length} tracks. :metal:`)
    } catch (err) {
      msg.reply(`I couldn't get to the page on Obsidian portal:\n\`\`\`\n${err.stack}\n\`\`\`\n`)
    }
  })
}
