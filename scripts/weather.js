...
function tempToColor (temp) {
  ...
  return tinycolor.mix(colors.get(lowBound), colors.get(highBound), temperatureBlend).toHexString()
}
...
function iconToEmoji (icon) {
  switch (icon) {
    ...
    case 'partly-cloudy-day': return 'partly_sunny'
  ...
  robot.respond(/weather *(.+)/i, async function (msg) {
    try {
      ...
      const attachment = {
        ...
        fields: [
          { title: 'Currently', value: `${forecastResult.minutely.summary} ${forecastResult.currently.temperature} °F, feels like ${forecastResult.currently.apparentTemperature} °F. :${currentlyEmoji}:` },
          { title: 'Today', value: `${forecastResult.daily.data[0].summary} High: ${forecastResult.daily.data[0].temperatureMax} °F Low: ${forecastResult.daily.data[0].temperatureMin} °F. :${todayEmoji}:` },
          { title: 'Tomorrow', value: ` ${forecastResult.daily.data[1].summary} High: ${forecastResult.daily.data[1].temperatureMax} °F Low: ${forecastResult.daily.data[1].temperatureMin} °F. :${tomorrowEmoji}:` }
        ],
