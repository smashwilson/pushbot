...
module.exports = function (robot) {
  ...
  robot.respond(/eternal *(.+)/i, function (msg) {
    ...
    const lookup = process.env.HUBOT_ETERNAL_LOOKUP || 'https://s3.amazonaws.com/eternaldecks/cards/<card_name>.png'

    if (found.length <= 5) {
      for (const card of found) {
        const url = lookup.replace('<card_name>', encodeURIComponent(card.name))
        msg.send(url)
      }
    } else {
