...
const betrayImmune = _.map((process.env.HUBOT_BETRAY_IMMUNE || '').split(/,/), line => line.trim())

const targetFrom = function (msg, matchNo) {
  if (matchNo == null) { matchNo = 1 }
  if (msg.match[matchNo]) { return msg.match[matchNo] } else { return msg.message.user.name }
}
...
`
  ...
  robot.respond(/nope\s*$/i, msg => msg.send('http://www.reactiongifs.com/wp-content/uploads/2013/06/nope.gif'))
  ...
  robot.hear(/\b(?:tit downwards|breasted boobily)\b/i, msg => msg.send('http://imgur.com/TRAPYBX'))

  robot.respond(/embiggen\s+([^]+)/i, msg => msg.send(`:muscle-left: ${msg.match[1]} :muscle-right:`))
