...
module.exports = function (robot) {
  ...
  function characterNameFrom (msg) {
    if (msg.match[1]) {
      ...
      if (!dmOnly(msg) && (msg.match[1] !== msg.message.user.name)) { return null }
      return msg.match[1]
    } else {
  ...
  function hasInitiativeTie (initiativeMap, username) {
    return Object.keys(initiativeMap.unresolvedTies).some(score => initiativeMap[score].includes(username))
  }
  ...
  robot.respond(/hp(?:\s+@(\S+))?(?:\s+(\+|-)?\s*(\d+))?/i, function (msg) {
    ...
    withCharacter(msg, (_existing, character) => {
      ...
      let finalhp = (() => {
        switch (op) {
          case '+': return inithp + amount
          case '-': return inithp - amount
          default: return amount
        }
      })()

      if (finalhp > character.maxhp) { finalhp = character.maxhp }
      character.currenthp = finalhp

      const lines = [`@${character.username}'s HP: ${inithp} :point_right: ${finalhp} / ${character.maxhp}`]
      if (finalhp <= 0) {
  ...
  robot.respond(/init(?:\s+@(\S+))?\s+(-?\d+)/, function (msg) {
    ...
    const initiativeMap = robot.brain.get('dnd:initiativeMap') || INITIATIVE_MAP_DEFAULT
    withCharacter(msg, (_existing, character) => {
      const existing = initiativeMap.scores.find(each => each.username === character.username)
      ...
      const lines = [`@${character.username} will go at initiative count ${score}.`]
      if (hasInitiativeTie(initiativeMap, character.username)) { lines.push("It's a tie!") }
      msg.send(lines.join('\n'))
  ...
  robot.respond(/init\s+reroll(?:\s+@(\S+))?\s+(-?\d+)/i, function (msg) {
    ...
    const initiativeMap = robot.brain.get('dnd:initiativeMap') || INITIATIVE_MAP_DEFAULT
    let username = null
    withCharacter(msg, (_existing, character) => { username = character.username })
  ...
  robot.respond(/init\s+next/i, function (msg) {
    ...
    if (!reportInitiativeTies(msg, initiativeMap)) { return }

    if (initiativeMap.current) {
      nextCount = initiativeMap.current + 1
      if (nextCount >= initiativeMap.scores.length) { nextCount = 0 }
    } else {
      nextCount = 0
    }
  ...
  robot.respond(/init\s+report/i, function (msg) {
    ...
    for (let each of initiativeMap.scores) {
      const prefix = (initiativeMap.current || 0) === i ? ':black_square_button:' : ':black_square:'
