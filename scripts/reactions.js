...
module.exports = function(robot) {
  ...
  robot.respond(/reactions\s*(?:@?(\S+))?/i, msg => {
    ...
    TallyMap.reactionsReceived(robot).topForUser(
      uid,
      10,
      (err, reaction, tally) => {
        if (err) {
  ...
  robot.respond(/toppun(?:\s*@?(\S+))?/i, msg => {
    if (msg.match[1]) {
      ...
      const tally =
        TallyMap.reactionsReceived(robot).forUser(uid).beachball || "no";
      msg.send(`<@${uid}> has been struck by *${tally}* :beachball:.`);
      return;
    }

    const lines = ["*Top 10 punmakers*"];
    TallyMap.reactionsReceived(robot).topForKey(
      "beachball",
      10,
      (err, uid, tally) => {
        if (err) {
